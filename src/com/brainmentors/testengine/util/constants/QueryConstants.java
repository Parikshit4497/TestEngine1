...
public interface QueryConstants {
  ...
  String REGISTER_SQL = "insert into user_mst(firstname,lastname,emailid,phoneno,password,confirmpassword,selectcity,selectcollege,selectstream,selectusertype,gender,dateofbirth,collegeid,userid) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	String fetchUnmpapped = " select  qid,name ,ans1,ans2,ans3,ans3,ans4,rans,score from question where status='NO'";
    ...
    String firstMapping="select qid,name,ans1,ans2,ans3,ans4,rans,score from question";
    String StatusUpdate="update question set status=? where qid=? ";
     String fetchMappedQuestion="select question.qid,question.name ,question.ans1,question.ans2,question.ans3,question.ans3,question.ans4,question.rans,question.score from question,test,testmap where test.testid=testmap.testid and testmap.qid=question.qid  and testname=?";
}